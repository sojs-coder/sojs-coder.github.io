<script lang="ts">
  ...
  const images = [Mural1, Mural2, Hero, Journal, AdHocHome, Dragonball, FlappySprite, ForgeEngineNodeEditor, ForgeEditor, Minecraft, Physics, Roommate, TheAdvocate, SpriteBuilder];
  ...
  function nextImage() {
    ...
    const indicesToChooseFrom = availableIndices.length > 0 
      ? availableIndices 
      : images.map((_, index) => index).filter(index => index !== currentIndex);
    
    // Randomly select from available indices
    const randomIndex = Math.floor(Math.random() * indicesToChooseFrom.length);
